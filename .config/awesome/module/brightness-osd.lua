...
bri_osd_slider:connect_signal(
	...
	function()
		...
		emit('widget::brightness:update', brightness_level)

		if awful.screen.focused().show_bri_osd then
			emit('module::brightness_osd:show',
				true
			)
		end
	end
...
screen.connect_signal(
	...
	function(s)
		...
		s.brightness_osd_overlay:connect_signal(
			...
			function()
				awful.screen.focused().show_bri_osd = true
				emit('module::brightness_osd:rerun')
			end
		)
	end
)
...
connect('module::brightness_osd:show',
	function(bool)
		placement_placer()
		awful.screen.focused().brightness_osd_overlay.visible = bool
		if bool then
			emit('module::brightness_osd:rerun')
			emit('module::volume_osd:show', false)
		else
			if hide_osd.started then
				hide_osd:stop()
			end
