...
local function draw_shape(s, collection, current_idx, icon_f, y, text_height)
  debug.print_warning(string.format('Index type: %s', type(current_idx)))
  debug.print_warning(string.format('Collection type: %s', type(collection)))

  if current_idx == nil then
    return
  end
  debug.print_warning(string.format('Index: %d', current_idx))
  debug.print_warning(string.format('Collection length: %d', #collection))

  debug.dump(collection, 'collection', 3)
