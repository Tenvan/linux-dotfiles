...
-- {{{ Key bindings
local globalkeys = my_table.join(awful.key({ modkey, shiftkey }, 'r', function()
    awesome.restart()
end, {
    ...
    awful.key({ altkey }, escapekey, awful.tag.history.restore, {
        description = 'go back',
        group = kgTag
    ...
    awful.key({ modkey }, 'F2', function()
        ...
        group = kgMenus
    }), awful.key({ controlkey, modkey }, 'z', function()
        awful.spawn.with_shell('recoll')
    end, {
        description = 'Recoll (lokale Textsuche)',
        group = kgMenus
    ...
    awful.key({ controlkey, altkey }, 'k', function()
        awful.spawn(
